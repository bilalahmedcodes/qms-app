...
@section('content')
    <div class="search-area">
        ...
        <form action="{{ route('reports.team-leads') }}" method="get" autocomplete="off" id="search"
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4 mb-3">
                                    ...
                                    <select name="team_lead_id" class="form-control select2" style="width: 100%;">
                                        <option value="-1">Select Option</option>
                                        @foreach ($team_leads as $team_lead)
                                            <option value="{{ $team_lead->hrms_id }}"
                                                @if ($team_lead->hrms_id == $team_lead_id) selected @endif>
                                                {{ $team_lead->hrms_id }} - {{ $team_lead->name }} -
                                                {{ $team_lead->campaign->name ?? '' }}</option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="type" value="team_leads">
                                </div>
                                <div class="form-group col-md-4 mb-3">
                                ...
                                <div class="form-group col-md-4 mb-3">
                                    ...
                                    <a href="{{ route('reports.team-leads', ['type' => 'teamLeads']) }}"
                                        class="ml-5">Clear
                                        Search</a>
                                </div>
                            </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Team Lead Name</th>
                            ...
                            <tbody>
                                <?php $counter = 1; ?>
                                @foreach ($teamLeadReports as $key => $item)
                                    <tr>
                                        <td class="text-center">{{ $teamLeadReports->firstItem() + $key }}</td>
                                        <td class="text-center">{{ $item->teamLead->hrms_id }}</td>
                                        <td class="text-center">{{ $item->teamLead->name }}</td>
                    ...
                    {{ $teamLeadReports->appends(request()->input())->links('vendor.pagination.default') }}
                    <small>Showing {{ $teamLeadReports->firstItem() }} to {{ $teamLeadReports->lastItem() }} of
                        {{ $teamLeadReports->total() }}
                        entries</small>
                </div>
            </div>
        </div>
